 ...
 * internal discovery API paths from discovery-insights-internal-api-v1.yaml
 * ({@code GET /stations/{stationRef}/utilization} and
 * {@code GET /insights/trends}). The EXISTING public behavior of the
 * service is preserved unchanged: the public discovery API
 * ({@code /api/v1/stations}, {@code /api/v1/stations/{stationRef}} —
 * anonymous per the public contract's security schemes) and
 * {@code /actuator/health} stay permitAll, and {@code anyRequest} stays
 * permitAll so every other path keeps today's anonymous behavior. No
 * existing endpoint becomes authenticated by this change; the internal
 * controllers do not exist yet (test-only fixtures exercise the chain).</p>
 *
 * <p><b>Error contract</b> (RFC 9457 problem+json, base URI matches the
 * BFF/platform convention {@code https://api.evplatform.example/problems/}
 * — same convention PublicStationController uses for resource-not-found):</p>
 * <ul>
 ...
 *       {@code SCOPE_discovery:insights:read} authority.</li>
 * </ul>
 *
...
public class SecurityConfig {
    ...
    @Bean
    public SecurityFilterChain discoveryInternalApiSecurityFilterChain(
            HttpSecurity http,
        ...
        http
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers(HttpMethod.GET, "/stations/*/utilization")
                                .hasAuthority("SCOPE_discovery:insights:read")
                        .requestMatchers(HttpMethod.GET, "/insights/trends")
                                .hasAuthority("SCOPE_discovery:insights:read")
                        // ---- preserved existing behavior ----
                        .requestMatchers(HttpMethod.GET, "/api/v1/stations").permitAll()
                        .requestMatchers(HttpMethod.GET, "/api/v1/stations/{stationRef}")
                                .permitAll()
